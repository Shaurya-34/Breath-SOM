...
import { SOMParams, SOMNode, remoteStep, remoteReset, remoteSetParams, connectSOMWebSocket } from "@/lib/som";

interface SOMCanvasProps {
  params: SOMParams;
  resetKey: number;
  onNodesUpdate?: (nodes: SOMNode[], iteration: number) => void;
}

const DISP_LERP = 0.12;
const ROT_LERP  = 0.08;
...
const RIPPLE_LIFETIME = 180;
const RIPPLE_MAX      = 50;
const RIPPLE_SPREAD0  = 2.5;
const RIPPLE_EXPAND   = 0.05;
const RIPPLE_WAVE_K   = 0.20;
const RIPPLE_WAVE_W   = 0.035;
...
function wColor(w: number[]): [number, number, number] {
  return hslToRgb((180+(w[0] ?? 0)*120)/360, (70+(w[1] ?? 0)*30)/100, (40+(w[2] ?? 0)*35)/100);
}
...
function rippleZ(
  ...
  const waveW = RIPPLE_WAVE_W * speedMul;
...
function drawScene(
  ...
  const zScale = (p.zScale ?? 1.0) * zSz * 4;
  ...
  const pts = nodes.map((n, i) => {
    ...
    const gx = (col / (gW - 1) - 0.5) * gridSz + ((n.weights[0] ?? 0) - 0.5) * dispSz;
    const gy = (row / (gH - 1) - 0.5) * gridSz + ((n.weights[1] ?? 0) - 0.5) * dispSz;

    const speedMul = p.animationSpeed ?? 1.0;
    const rZ = rippleZ(col, row, ripples, fc, zScale, speedMul);
    const gz = ((n.weights[2] ?? 0) - 0.5) * zSz + rZ;
...
const SOMCanvas = ({ params, resetKey, onNodesUpdate }: SOMCanvasProps) => {
  ...
  const [, setStatus] = useState<"loading" | "live" | "error">("loading");
  ...
  }, [resetKey, params.learningRate, params.neighborhoodRadius, params.epochs, params.gridSize, params.datasetType]);

  useEffect(() => {
    let wsActive = false;
    const disconnect = connectSOMWebSocket((data) => {
      wsActive = true;
      remoteRef.current = data.nodes;
      if (onNodesUpdate) onNodesUpdate(data.nodes, data.iteration);
      const peakDelta = Math.max(...data.delta, 0);
      if (peakDelta > 0.0001) {
      ...
      while (alive) {
        if (!wsActive) {
          try {
            ...
            if (onNodesUpdate) onNodesUpdate(r.nodes, r.iteration);
